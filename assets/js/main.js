	...
	fetch('./places.json').then(data=>data.json()).then((data)=>{
		for (let place of data.places){
			...
			if (Array.isArray(place.detailed)){
				detailed = `Places to visit in ${place.place} include:<br><ul>`
				for (let detailedplace of place.detailed){
					detailed+=`<li>${detailedplace.name}</li>`
					detailedplaces+=`<h3>${detailedplace.name}</h3>${detailedplace.description}`
				}
				detailed+=`</ul>`
			}
			elem.innerHTML=`<hr><h2>${place.place}</h2> <p>${place.description}</p>${detailed}${detailedplaces}`
